...
describe("POST /attendance/start", () => {
  ...
  it("should return 401 without token", async () => {
    const { status, data } = await request("POST", "/attendance/start", {
      classId: "someid",
    });
...
describe("End-to-End Attendance Flow", () => {
  it("should complete full attendance workflow", async () => {
    ...
    const classCreate = await request(
      "POST",
      "/class",
      {
    ...
    const startAttendance = await request(
      "POST",
      "/attendance/start",
      {
        classId: classId,
    ...
    const myAttendanceHttp = await request(
      "GET",
      `/class/${classId}/my-attendance`,
      null,
      student1Token,
